...
function event_management_blocks_admin_notice() {
    ...
    $event_management_blocks_dismissed = get_user_meta(get_current_user_id(), 'event_management_blocks_dismissed_notice', true);
    $event_management_blocks_current_page = '';
    if(isset($_GET['page'])) {
    	$event_management_blocks_current_page = admin_url( "admin.php?page=".sanitize_text_field($_GET["page"]));
    }

    // Display the notice only if not dismissed
    if (!$event_management_blocks_dismissed && $event_management_blocks_current_page != admin_url( "admin.php?page=wordclever-templates")){
        ?>
        		...
        		<div class="notice-content">
	        		<div class="notice-holder">
	                        ...
	                        <a href="javascript:void(0);" id="install-activate-button" class="button admin-button info-button">
							   <?php echo __('Getting started', 'event-management-blocks'); ?>
							</a>

							<script type="text/javascript">
							document.getElementById('install-activate-button').addEventListener('click', function () {
							    const event_management_blocks_button = this;
							    const event_management_blocks_redirectUrl = '<?php echo esc_url(admin_url("themes.php?page=event-management-blocks")); ?>';
