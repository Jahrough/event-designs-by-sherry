...
class Style
{
    ...
    function __get($prop)
    {
        ...
        if (!isset(self::$_defaults[$prop])) {
			throw new Exception( esc_html( "'$prop' is not a recognized CSS property." ) );
        }
    ...
    function get_font_family()
    {
        ...
        foreach ($families as $family) {
            ...
            if ($font) {
                if ($DEBUGCSS) {
					...
					print '(' . esc_html( $this->_props_computed["font_family"] ) . '.' . esc_html( $font_style ) . '.' . esc_html( $weight ) . '.' . esc_html( $subtype ) . ')';
        ...
        if ($font) {
            if ($DEBUGCSS) {
				print '(' . esc_html( $font ) . ")get_font_family]\n</pre>";
            }
            return $font;
        }
    ...
    */
    ...
    protected function _image($val)
    {
        ...
        if ($DEBUGCSS) {
            print "<pre>[_image\n";
            print_r($parsed_url);
			print esc_html( $this->_stylesheet->get_protocol() ) . "\n" . esc_html( $this->_stylesheet->get_base_path() ) . "\n" . esc_html( $path ) . "\n";
            print "_image]</pre>";;
        }
        return $path;
    }
    ...
    function debug_print()
    {
        ...
        print "      specified [\n";
        foreach ($this->_props as $prop => $val) {
			print '        ' . esc_html( $prop ) . ': ' . esc_html( preg_replace( "/\r\n/", ' ', print_r( $val, true ) ) );
