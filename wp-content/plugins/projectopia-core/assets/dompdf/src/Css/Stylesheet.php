                ...
                case "[":
                    ...
                    switch ($tok[$j]) {
                        ...
                        case "*":
                            ...
                            if ($tok[$j] !== "=") {
								throw new Exception( sprintf( 'Invalid CSS selector syntax: invalid attribute selector: %s', esc_html( $selector ) ) );
                            }
                    ...
                    if ($attr == "") {
						throw new Exception( esc_html( 'Invalid CSS selector syntax: missing attribute name' ) );
                    }
...
//    }
    ...
    protected function _image($val)
    {
        ...
        if ($DEBUGCSS) {
            print "<pre>[_image\n";
            print_r($parsed_url);
			print esc_html( $this->_protocol ) . "\n" . esc_html( $this->_base_path ) . "\n" . esc_html( $path ) . "\n";
            print "_image]</pre>";
        }
        return $path;
    }
          ...
          $important = preg_match("/(.*?)!\s*important/",$prop,$match);
            ...
            */
            ...
            if ($i === false) {
                ...
                if ($DEBUGCSS) print 'novalue' . esc_html($prop) . ')';
                continue;
            }

            $prop_name = rtrim(mb_strtolower(mb_substr($prop, 0, $i)));
            $value = ltrim(mb_substr($prop, $i + 1));
			if ($DEBUGCSS) print esc_html( $prop_name ) . ':=' . esc_html( $value ) . ( $important ? ' !IMPORTANT' : '' ) . ')';
    ...
    private function _parse_sections($str, $media_queries = [])
    {
        ...
        foreach ($sections as $sect) {
            ...
            foreach ($selectors as $selector) {
				...
				if ($DEBUGCSS) print '#' . esc_html( $selector ) . '#';
