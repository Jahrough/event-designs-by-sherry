...
class Helpers
{
    ...
    public static function imagecreatefrombmp( $filename, $context = null ) {
        ...
        // check for bitmap
        if ( $meta['type'] != 19778 ) {
            trigger_error('imagecreatefrombmp: ' . esc_html($filename) . ' is not a bitmap!', E_USER_WARNING);
            return false;
        }
        ...
        if ( $meta['imagesize'] < 1 ) {
            ...
            if ( $meta['imagesize'] < 1 ) {
                ...
                if ( $meta['imagesize'] < 1 ) {
                    trigger_error('imagecreatefrombmp: Can not obtain filesize of ' . esc_html($filename) . '!', E_USER_WARNING);
                    return false;
                }
            }
        }
        ...
        $vide = chr(0);
        $y = $meta['height'] - 1;
        $error = 'imagecreatefrombmp: ' . esc_html($filename) . ' has not enough data!';
            ...
            while ( $x < $meta['width'] ) {
                switch ( $meta['bits'] ) {
                    ...
                    case 1:
                        ...
                        break;
                    default:
						trigger_error('imagecreatefrombmp: ' . esc_html($filename) . ' has ' . esc_html($meta['bits']) . ' bits and this is not supported!', E_USER_WARNING);
                        return false;
                }
