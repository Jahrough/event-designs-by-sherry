    ...
    */
    ...
    public function loadHtmlFile( $file, $encoding = null ) {
        ...
        if ( ! in_array($protocol, $this->allowedProtocols) ) {
            throw new Exception( esc_html( "Permission denied on $file. The communication protocol is not supported." ) );
        }
        ...
        if ( $protocol == "" || $protocol === "file://" ) {
            ...
            if ( $chrootValid !== true ) {
               throw new Exception( esc_html( "Permission denied on $file. The file could not be found under the paths specified by Options::chroot." ) );
            }

            $ext = strtolower(pathinfo($realfile, PATHINFO_EXTENSION));
            if ( ! in_array($ext, $this->allowedLocalFileExtensions) ) {
                throw new Exception( esc_html( "Permission denied on $file. This file extension is forbidden" ) );
            }

            if ( ! $realfile ) {
                throw new Exception( esc_html( "File '$file' not found." ) );
            }
    ...
    public function render() {
        ...
        if ( $_dompdf_show_warnings && isset($_dompdf_warnings) ) {
            ...
            if ( $canvas instanceof CPDF ) {
                echo esc_html( $canvas->get_cpdf()->messages );
            }
            echo '</pre>';
            flush();
        }
    ...
    function __get( $prop ) {
        switch ( $prop ) {
            case 'version':
                return $this->version;
            default:
                throw new Exception( esc_html( 'Invalid property: ' . $prop ) );
        }
    }
}
