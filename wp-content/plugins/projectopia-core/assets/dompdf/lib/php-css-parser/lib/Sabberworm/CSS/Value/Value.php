...
abstract class Value implements Renderable {
	...
	public static function parseValue(ParserState $oParserState, $aListDelimiters = array()) {
		...
		foreach ($aListDelimiters as $sDelimiter) {
			...
			while (($iStartPosition = array_search($sDelimiter, $aStack, true)) !== false) {
				...
				$oList = new RuleValueList($sDelimiter, esc_html($oParserState->currentLine()));
	...
	public static function parseIdentifierOrFunction(ParserState $oParserState, $bIgnoreCase = false) {
		...
		if ($oParserState->comes('(')) {
			...
			$sResult = new CSSFunction($sResult, $aArguments, ',', esc_html($oParserState->currentLine()));
			$oParserState->consume(')');
		}
	...
	private static function parseMicrosoftFilter(ParserState $oParserState) {
		...
		$aArguments = Value::parseValue($oParserState, array(',', '='));
		return new CSSFunction($sFunction, $aArguments, ',', esc_html($oParserState->currentLine()));
	}
