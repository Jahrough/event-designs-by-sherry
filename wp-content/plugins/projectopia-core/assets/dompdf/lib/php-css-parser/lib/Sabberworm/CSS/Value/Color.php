	...
	public static function parse(ParserState $oParserState) {
		...
		if ($oParserState->comes('#')) {
			...
			if ($oParserState->strlen($sValue) === 8) {
				$aColor = array(
					'r' => new Size(intval($sValue[0] . $sValue[1], 16), null, true, esc_html($oParserState->currentLine())),
					'g' => new Size(intval($sValue[2] . $sValue[3], 16), null, true, esc_html($oParserState->currentLine())),
					'b' => new Size(intval($sValue[4] . $sValue[5], 16), null, true, esc_html($oParserState->currentLine())),
					'a' => new Size(round(self::mapRange(intval($sValue[6] . $sValue[7], 16), 0, 255, 0, 1), 2), null, true, esc_html($oParserState->currentLine()))
				);
			} else {
				$aColor = array(
					'r' => new Size(intval($sValue[0] . $sValue[1], 16), null, true, esc_html($oParserState->currentLine())),
		...
		return new Color($aColor, esc_html($oParserState->currentLine()));
	}
