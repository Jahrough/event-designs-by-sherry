...
abstract class CSSList implements Renderable, Commentable {
	...
	public static function parseList(ParserState $oParserState, CSSList $oList) {
		...
		if(!$bIsRoot && !$bLenientParsing) {
			throw new SourceException("Unexpected end of document", esc_html($oParserState->currentLine()));
		}
	}
	...
	private static function parseListItem(ParserState $oParserState, CSSList $oList) {
		...
		if ($oParserState->comes('@')) {
			...
			if($oAtRule instanceof Charset) {
				if(!$bIsRoot) {
					throw new UnexpectedTokenException('@charset may only occur in root document', '', 'custom', esc_html($oParserState->currentLine()));
				}
				if(count($oList->getContents()) > 0) {
					throw new UnexpectedTokenException('@charset must be the first parseable token in a document', '', 'custom', esc_html($oParserState->currentLine()));
				}
			...
			if ($bIsRoot) {
				if ($oParserState->getSettings()->bLenientParsing) {
					...
					throw new SourceException("Unopened {", esc_html($oParserState->currentLine()));
				}
			} else {
				return null;
			}
	...
	private static function parseAtRule(ParserState $oParserState) {
		...
		$iIdentifierLineNum = esc_html($oParserState->currentLine());
		$oParserState->consumeWhiteSpace();
		if ($sIdentifier === 'import') {
			...
			if ($sPrefix !== null && !is_string($sPrefix)) {
				throw new UnexpectedTokenException( esc_html( 'Wrong namespace prefix' ), esc_html( $sPrefix ), esc_html( 'custom' ), esc_html( $iIdentifierLineNum ) );
			}
			if (!($mUrl instanceof CSSString || $mUrl instanceof URL)) {
				throw new UnexpectedTokenException( esc_html( 'Wrong namespace url of invalid type' ), esc_html( $mUrl ), esc_html( 'custom' ), esc_html( $iIdentifierLineNum ) );
			}
			...
			if (substr_count($sArgs, "(") != substr_count($sArgs, ")")) {
				if($oParserState->getSettings()->bLenientParsing) {
					...
					throw new SourceException("Unmatched brace count in media query", esc_html($oParserState->currentLine()));
				}
			}
			$bUseRuleSet = true;
