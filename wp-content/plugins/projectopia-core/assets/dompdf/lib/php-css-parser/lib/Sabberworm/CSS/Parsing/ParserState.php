...
class ParserState {
	...
	public function parseIdentifier($bIgnoreCase = true) {
		...
		if ($sResult === null) {
throw new UnexpectedTokenException( esc_html( $sResult ), esc_html( $this->peek(5) ), esc_html( 'identifier' ), esc_html( $this->iLineNo ) );
		}
		$sCharacter = null;
	...
	public function consume($mValue = 1) {
		if (is_string($mValue)) {
			...
			if (!$this->streql($this->substr($this->iCurrentPosition, $iLength), $mValue)) {
				throw new UnexpectedTokenException( esc_html( $mValue ), esc_html( $this->peek( max( $iLength, 5 ) ) ), esc_html( $this->iLineNo ) );
			...
			if ($this->iCurrentPosition + $mValue > $this->iLength) {
				throw new UnexpectedTokenException( esc_html( $mValue ), esc_html( $this->peek(5) ), esc_html( 'count' ), esc_html( $this->iLineNo ) );
			}
