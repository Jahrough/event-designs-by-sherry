...
function pto_save_post_cqpim_tasks( $post_id, $post ) {
	...
	// phpcs:ignore WordPress.Security.NonceVerification.Missing
	if( isset( $_POST['task_status'] ) && !empty( sanitize_text_field(wp_unslash($_POST['task_status'])) )) {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		update_post_meta($post_id,'task_status',sanitize_text_field(wp_unslash($_POST['task_status'])));
	}
