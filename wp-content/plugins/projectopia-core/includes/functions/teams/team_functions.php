...
function pto_reset_password() {
	...
	if ( ! isset( $_POST['team_password_change_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['team_password_change_nonce'] ) ), 'team_password_change_action' ) ) {
		pto_send_json( array( 
			'error'   => true,
			'message' => '<div class="cqpim-alert cqpim-alert-warning alert-display">' . esc_html__( 'Security check failed.', 'projectopia-core' ) . '</div>',
		) );
	}

	if ( ! current_user_can( 'cqpim_reset_team_passwords' ) || ! current_user_can( 'cqpim_reset_client_passwords' ) ) {
        pto_send_json( array(
            'error'   => true,
            'message' => '<div class="cqpim-alert cqpim-alert-warning alert-display">' . esc_html__( 'You do not have permission to perform this action.', 'projectopia-core' ) . '</div>',
        ) );
    }
