...
function pto_edit_income_graph() {
	...
	if ( $type == 'type' ) {
		...
		$invoice_year = isset($_POST['date']) ? sanitize_text_field(wp_unslash( $_POST['date'])) : gmdate('Y');
...
function pto_generate_pdf_invoice( $invoice_id, $invoice_no ) {
	...
	$base_url = get_the_permalink($invoice_id);
	$url = add_query_arg(
		array(
			'pwd'      => md5($invoice->post_password),
			'pto-page' => 'print',
			'mode'     => 'download'
		),
		$base_url
	...
	// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents, WordPress.Security.EscapeOutput.OutputNotEscaped
