...
$selected = ! empty($invoice_year) ? $invoice_year : gmdate('Y');
...
if ( empty( $invoice_year ) ) {
    pto_set_transient( 'invoice_year', gmdate( 'Y') );
}
...
<div class="card ">
    <div class="card-header d-block d-md-flex ">
        <div class="card-header-info d-flex align-items-center">
            <img src="<?php echo esc_url( PTO_PLUGIN_URL ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>/assets/admin/img/taskIcon.png" alt="" class="img-fluid mr-2" />
    ...
    <div class="card-header d-block d-md-flex mt-3">
        <!-- Widget Header -->
        <div class="card-header-info d-flex align-items-center">
            <img src="<?php echo esc_url( PTO_PLUGIN_URL ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>/assets/admin/img/revenue.svg" alt="" class="img-fluid mr-2" />
            <div class="content">
                <h4 class="mb-0">
                    <?php echo esc_html(pto_format_currency(esc_attr($current_month_income))); ?>
                </h4>
            ...
            <div class="selectDropdown d-block d-sm-flex flex-wrap align-items-center">
                <div class="dropdownInner padding-ls-small mr-3 mt-sm-2" data-prev-content="Year">
                    <select id="income_control_date" class="selectDropdown-init form-control">
                        ...
                        <?php $date = gmdate('Y', strtotime("-1 year")); ?>
                        <option value="<?php echo esc_attr( $date ); ?>" <?php if ( $date == $selected ) { ?> selected="selected"<?php } ?>><?php echo esc_html( $date ); ?></option>
                        <?php $date = gmdate('Y', strtotime("-2 years")); ?>
                        <option value="<?php echo esc_attr( $date ); ?>" <?php if ( $date == $selected ) { ?> selected="selected"<?php } ?>><?php echo esc_html( $date ); ?></option>
                        <?php $date = gmdate('Y', strtotime("-3 years")); ?>
                        <option value="<?php echo esc_attr( $date ); ?>" <?php if ( $date == $selected ) { ?> selected="selected"<?php } ?>><?php echo esc_html( $date ); ?></option>
                    </select>
                </div>
