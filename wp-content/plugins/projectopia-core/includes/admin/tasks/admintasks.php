 ...
 * Admin Tasks Page 
 *
 * This is admin task page showing list of tasks for admin.
...
//Register admin tasks as sub menu page.
add_action( 'admin_menu' , 'register_pto_task_admin_page', 29 ); 
function register_pto_task_admin_page() {
	$mypage = add_submenu_page( 
				'pto-dashboard',
				__('All Tasks (Admin)', 'projectopia-core'),           
				'<span class="pto-sm-hidden">' . esc_html__('All Tasks (Admin)', 'projectopia-core') . '</span>',           
					...
					<div class="card all-project-card">
						<div class="card-header d-block d-md-flex">
							<div class="card-header-info d-flex align-items-center">
								<img src="<?php echo esc_url( PTO_PLUGIN_URL ) .'/assets/admin/img/taskIcon.png'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" class="img-fluid mr-2" />
								<h5 class="mb-0"><?php esc_html_e('All Tasks (Admin)', 'projectopia-core'); ?> </h5>
							</div>
										...
										<tbody>
											<?php foreach ( $ordered as $task ) {
												...
												<tr>
													...
													<td>
														<div class="dropdownInner padding-ls-medium pr-3 mt-2" data-prev-content="Own">
															<select class="selectDropdown-init form-control admin_task_assignee" data-task="<?php echo esc_attr( $task->ID ); ?>">
																...
																<option value="<?php echo esc_attr( $client_id ); ?>" <?php selected($task_owner_id, $client_id); ?>><?php echo esc_html( $client_contact_name ); ?></option> </optgroup><?php }
																...
																<?php
																} elseif ( $task_owner_id != $client_id ) { ?>	
																	<option value="<?php echo esc_attr( $task_owner ); ?>" <?php selected( $task_owner_id, $task_owner_id ); ?>><?php echo esc_html( $team_name ); ?></option>
																<?php } ?>
															</select>
														</div>
														<?php echo wp_kses_post( $watching ); ?>
													</td>
													...
													<?php if ( pto_has_addon_active_license( 'pto_te', 'timeentries' ) ) { ?>
														...
														<td><button class="add_timer legacy_button border-green font-green cqpim_button cqpim_xs_button op" value="<?php echo esc_attr( $task->ID ); ?>"><i class="fa fa-clock-o" aria-hidden="true" title="<?php esc_html_e('Add Time', 'projectopia-core'); ?>"></i></button></td>
													<?php } ?>
													<td>
	...
	<div style="display:none">
		<div id="add-time-div" class="add-time-div">
			<div style="padding: 12px;min-width: 430px;">
				...
				<div class="form-group">
					<label for="timer_note"><?php esc_html_e('Add notes', 'projectopia-core'); ?></label>
					<div class="input-group">
