	...
	<ul id="cd-head-actions">
		...
		<?php if ( ! empty($messaging) ) { ?>
			<li class="desktop_items">
				<span class="cqpim_icon">
					<?php 
						$messages_url = add_query_arg('pto-page', 'messages', get_the_permalink($client_dash));
					?>
					<a <?php if (!empty($unread_qty)) { echo 'class="cqpim_active"'; } ?> href="<?php echo esc_url($messages_url); ?>">
						<i class="fa fa-envelope-open cqpim_tooltip" aria-hidden="true" title="<?php esc_html_e('Messages', 'projectopia-core'); ?>"></i>
					</a>
		...
		if ( $client_settings == 1 ) { ?>
			<li class="desktop_items">
				<span class="cqpim_icon">
					<?php
						$settings_url = add_query_arg('pto-page', 'settings', get_the_permalink($client_dash));
					?>
					<a href="<?php echo esc_url($settings_url); ?>">
						<i class="fa fa-sliders cqpim_tooltip" aria-hidden="true" title="<?php esc_html_e('Edit my Profile', 'projectopia-core'); ?>"></i>
					</a>
				</span>
			</li>
		<?php } ?>
		<li class="desktop_items">
			<span class="cqpim_icon">
				<?php
				$client_files_url = add_query_arg('pto-page', 'client-files', get_the_permalink($client_dash));
				?>
				<a href="<?php echo esc_url($client_files_url); ?>">
					<i class="fa fa-file cqpim_tooltip" aria-hidden="true" title="<?php esc_html_e('Client Files', 'projectopia-core'); ?>"></i>
				</a>			
			</span>
		</li>
		...
		if ( $client_settings == 1 ) { ?>
			<li class="contacts desktop_items">
				<span class="cqpim_icon">
					<?php
						$contacts_url = add_query_arg('pto-page', 'contacts', get_the_permalink($client_dash));
					?>
					<a href="<?php echo esc_url($contacts_url); ?>">
						<i class="fa fa-users cqpim_tooltip" aria-hidden="true" title="<?php esc_html_e('Contacts', 'projectopia-core'); ?>"></i>
					</a>				
				</span>
			</li>
		<?php } ?>
...
<div id="cqpim_admin_title">
	...
	<?php if ( ! empty($quote_form) && get_option('enable_quotes') == 1 ) { ?>
		<a href="<?php echo esc_url(add_query_arg('pto-page', 'quote_form', get_the_permalink($client_dash))); ?>"><i class="fa fa-angle-double-right" aria-hidden="true"></i> <?php esc_html_e('Request a Quote', 'projectopia-core'); ?></a>
	<?php } ?>
	...
	<?php if ( get_option('cqpim_enable_faq_dash') ) { ?>
		<?php
			$faq_url = add_query_arg('pto-page', 'faq', get_the_permalink($client_dash));
		?>
		<a href="<?php echo esc_url($faq_url); ?>">
			<i class="fa fa-question-circle" aria-hidden="true"></i><?php esc_html_e('FAQ', 'projectopia-core'); ?>
		</a>	
	<?php } ?>	
	...
	<?php if ( $post->post_type == 'cqpim_project' || $post->post_type == 'cqpim_task' || $post->post_type == 'cqpim_bug' ) { 
		...
		if ( ! empty($project_details) ) { ?>
			<?php
				$project_info_url = add_query_arg(
					array(
						'pto-page' => 'summary',
						'sub' => 'info'
					),
			...
			?>
			<a href="<?php echo esc_url($project_info_url); ?>">
				<i class="fa fa-info-circle" aria-hidden="true"></i><?php esc_html_e('Project Information', 'projectopia-core'); ?>
			</a>		
		<?php } ?>
		<?php $checked = get_post_meta($ppid, 'contract_status', true);
		if ( ! empty($checked) && $checked == 1 ) { ?>
			<?php
				$contract_url = add_query_arg('pto-page', 'contract', get_the_permalink($ppid));
			?>
			<a href="<?php echo esc_url($contract_url); ?>">
				<i class="fa fa-file-text" aria-hidden="true"></i><?php esc_html_e('View Contract', 'projectopia-core'); ?>
			</a>
		<?php } ?>
		<?php
		...
		<a href="<?php echo esc_url($updates_url); ?>">
			<i class="fa fa-pencil-square-o" aria-hidden="true"></i><?php esc_html_e('Updates & Progress', 'projectopia-core'); ?>
		</a>
		<a href="<?php echo esc_url($milestones_url); ?>">
			<i class="fa fa-tasks" aria-hidden="true"></i><?php esc_html_e('Milestones & Tasks', 'projectopia-core'); ?>
		</a>

		<?php 
			$active = get_post_meta($ppid, 'bugs_activated', true);
			if ( pto_has_addon_active_license( 'pto_bugs', 'bugs' ) && $active == true ) {
				$bugs_url = add_query_arg(
					array(
						'pto-page' => 'summary',
						'sub' => 'bugs'
					),
				...
				?>
				<a href="<?php echo esc_url($bugs_url); ?>">
					<i class="fa fa-bug" aria-hidden="true"></i><?php esc_html_e('Bugs', 'projectopia-core'); ?>
				</a>			
			<?php } 
			...
			<a href="<?php echo esc_url($messages_url); ?>">
				<i class="fa fa-envelope" aria-hidden="true"></i><?php esc_html_e('Messages', 'projectopia-core'); ?>
			</a>
			<a href="<?php echo esc_url($files_url); ?>">
				<i class="fa fa-file" aria-hidden="true"></i><?php esc_html_e('Files', 'projectopia-core'); ?>
			</a>
			<?php if ( get_option('disable_invoices') != 1 ) { ?>
				<a href="<?php echo esc_url($invoices_url); ?>">
					<i class="fa fa-credit-card-alt" aria-hidden="true"></i><?php esc_html_e('Costs & Invoices', 'projectopia-core'); ?>
				</a>
			<?php } else { ?>
				<a href="<?php echo esc_url($invoices_url); ?>">
					<i class="fa fa-credit-card-alt" aria-hidden="true"></i><?php esc_html_e('Costs', 'projectopia-core'); ?>
				</a>
			<?php } ?>
	...
	if ( $post->post_type == 'cqpim_quote' ) {
		...
		?>
		<?php
		$quote_messages_url = add_query_arg(
			array(
				'pto-page' => 'quote',
				'sub' => 'messages'
			),
		...
		?>
		<a href="<?php echo esc_url($quote_messages_url); ?>">
			<i class="fa fa-envelope" aria-hidden="true"></i><?php esc_html_e('Messages', 'projectopia-core'); ?>
		</a>
	<?php } ?>
</div>
