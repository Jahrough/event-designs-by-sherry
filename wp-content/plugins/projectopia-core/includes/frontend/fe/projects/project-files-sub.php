	...
	<div class="cqpim-dash-item-full grid-item tasks-box">
		<div class="cqpim_block">
			<div class="cqpim_block_title">
				<div class="caption">
					<span class="caption-subject font-green-sharp sbold"><?php esc_html_e('Project Files', 'projectopia-core'); ?></span>
				</div>	
			</div>
			<?php
				...
				if ( ! $all_attached_files ) {
					echo '<p style="padding:20px">' . esc_html__('There are no files uploaded to this project.', 'projectopia-core') . '</p>';
				} else {
			...
			<form id="project_fe_files">
				...
				<select id="task_id">
					...
					if ( empty($project_elements) ) {
						...
						foreach ( $ordered as $key => $element ) {
							echo '<optgroup label="' . esc_html( $element['title'] ) . '">';
								// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key, WordPress.DB.SlowDBQuery.slow_db_query_meta_query, WordPress.DB.SlowDBQuery.slow_db_query_meta_value
								$args = array(
									'post_type'      => 'cqpim_tasks',
									'posts_per_page' => -1,
				...
				<input type="submit" id="client_fe_files_submit" class="cqpim_button font-white bg-blue rounded_2 op" value="<?php esc_html_e('Submit Files', 'projectopia-core'); ?>">
			</form>
		</div>
	</div>
</div>