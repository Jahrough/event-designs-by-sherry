	...
	<div class="masonry-grid">
		...
		<div class="cqpim-dash-item-triple grid-item">
			<div class="cqpim_block">
				<div class="cqpim_block_title">
					<div class="caption">
						<span class="caption-subject font-green-sharp sbold"><?php esc_html_e('Line Items', 'projectopia-core'); ?></span>
					</div>
				</div>
				<br />
		...
		<div class="cqpim-dash-item-double grid-item">
			<div class="cqpim_block">
				<div class="cqpim_block_title">
					<div class="caption">
						<span class="caption-subject font-green-sharp sbold"><?php esc_html_e('Invoice Details', 'projectopia-core'); ?> </span>
					</div>
				</div>
				<?php $now = time();
				if ( empty($on_receipt) ) {
				...
				<span class="invoice_to">
					...
					<p><span><?php echo esc_html( get_option('secondary_sales_tax_name') ); ?> <?php esc_html_e('Number', 'projectopia-core'); ?>:</span> <span><?php echo esc_html( $stax_reg ); ?></span></p>						
						<?php } ?>
					<p><span><?php esc_html_e('Invoice #:', 'projectopia-core'); ?></span> <span><?php echo esc_html( $invoice_id ); ?></span></p>
				...
				<div>
					<?php if ( empty($invoice_details['paid']) ) { 
						...
						$n_ideal = get_option('client_invoice_stripe_ideal');
						...
						if ( ! empty($stripe) && ! empty($user->ID) || ! empty($twocheck) && ! empty($user->ID) || ! empty($paypal) && ! empty($user->ID) ) {    
							echo '<div id="stripe-pay" style="display:none">';
								if ( ! empty( $allow_partial ) ) {
									...
									echo '<span style="font-weight:bold">' . esc_html__('Amount to Pay', 'projectopia-core') . ' (' . esc_html( pto_calculate_currency($post->ID) ) . '): </span> ';
									echo '<input style="padding:5px; width:100px; margin:0 10px;" type="text" id="amount_to_pay" value="' . esc_attr( $amount_to_pay ) . '" />';
									echo ' <button class="cqpim_button cqpim_small_button rounded_2 bg-blue font-white" id="save_amount">' . esc_html__('Update', 'projectopia-core') . '</button><span id="amount_spinner" class="ajax_spinner" style="display:none"></span><br />';
									echo '</div>';                          
								}   
								/* translators: %s: Payment Amount */
								echo '<button style="font-size:14px" class="cqpim_button cqpim_button_link font-white bg-blue block mt-10 rounded_2 block" id="cqpim_pay_now">' . sprintf(esc_html__('Pay %s Now', 'projectopia-core'), esc_html(pto_calculate_currency($post->ID, $amount_to_pay))) . '</button>';
							echo '</div>'; ?>
									...
									<ul>
										...
										<?php if ( ! empty( $stripe ) ) { ?>
											<li>                                        <?php                                                                                                                    
												...
												$c_current_user = wp_get_current_user();
												$client_id = pto_get_client_from_userid( $c_current_user );
												...
												$client_postcode = isset( $client_details['client_postcode'] ) ? $client_details['client_postcode'] : '';
												$client_telephone = isset( $client_details['client_telephone'] ) ? $client_details['client_telephone'] : '';
												$client_email = isset( $client_details['client_email'] ) ? $client_details['client_email'] : $c_current_user->user_email;
												...
												<button class="cqpim_button cqpim_button_link font-white bg-blue block mt-10 rounded_2 block" id="pto-submit-payment" data-secret="<?php echo esc_attr( $intent->client_secret ); ?>">Pay With Stripe</button>
										...
										<?php } ?>
										<?php if ( ! empty($n_ideal) && ! empty($stripe) ) { ?>
											<li>
												<br />
												<img style="max-width:60px" src="<?php echo esc_url( PTO_PLUGIN_URL ) . '/img/ideal_logo.png'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" id="ideal_trigger" />													
											</li>
										<?php } ?>
				...
				<a class="cqpim_button cqpim_button_link font-white bg-blue block mt-10 rounded_2 block" href="<?php echo esc_url(add_query_arg('pto-page', 'print', get_the_permalink())); ?>" target="_blank"><?php esc_html_e('View Printable Invoice', 'projectopia-core'); ?></a>
                <a class="cqpim_button cqpim_button_link font-white bg-blue block mt-10 rounded_2 block" href="<?php echo esc_url(add_query_arg('download_pdf', '1', get_the_permalink())); ?>"><?php esc_html_e('Download PDF Invoice', 'projectopia-core'); ?></a>				
			</div>
		</div>		
	</div>
	<?php if ( ! empty($n_ideal) ) { 
		$user = wp_get_current_user(); ?>
				...
				<div id="ideal_form">
					...
					<form id="payment-form">
						...
						<div class="btn-container">
							<button class="cqpim_button bg-blue font-white rounded_2 op"><?php esc_html_e('Submit Payment', 'projectopia-core'); ?></button>
							<button class="cqpim_button bg-blue font-white rounded_2 op go-back"><?php esc_html_e('Back', 'projectopia-core'); ?></button>
						</div>
