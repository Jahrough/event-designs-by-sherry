...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    private function validateConfig($config)
    {
        ...
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('api_key must be null or a string'));
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('client_id must be null or a string'));
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('stripe_account must be null or a string'));
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('stripe_version must be null or a string'));
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('api_base must be a string'));
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('connect_base must be a string'));
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \Stripe\Exception\InvalidArgumentException(esc_html('files_base must be a string'));
        }
