...
if ( $p_mode == 'print' ) { ?>
	...
	<head>
		...
		<style>
			...
			#content{margin: 0px auto;width: 800px;padding: 0px;}
...
<body <?php body_class(); ?>>
	<div style="background: #fff;text-align: left" id="content" role="main">
		<?php if ( current_user_can( 'edit_cqpim_invoices' ) OR $client_user_id == $user_id OR $pass == md5($post->post_password) OR in_array($user->ID, $client_ids) ) { ?>
			<table style="width:100%;background:<?php echo esc_attr( $main_colour ); ?>; border-collapse:collapse">
				<tr>
					<td style="padding:15px;">
						<?php if ( ! empty($inv_logo['cqpim_invoice_logo']) ) { ?>
							<img src="<?php echo isset($inv_logo['cqpim_invoice_logo']) ? esc_url( $inv_logo['cqpim_invoice_logo'] ) : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" style="max-height: 150px;" />
						<?php } else { ?>
							<img src="<?php echo isset($logo['company_logo']) ? esc_url( $logo['company_logo'] ) : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" style="max-height: 150px;" />
						<?php } ?>
					</td>
					<td style="padding:15px 25px;text-align: right;">
						<p class="white" style="color:#fff; font-size:12px"><?php echo wp_kses_post( nl2br( $company_address ) ); ?> <?php echo esc_html( $company_postcode ); ?></p>
			...
			if ( empty($on_receipt) ) {
				if ( empty($invoice_details['paid']) ) {
					if ( $terms_over ) {
						if ( $now > $terms_over ) {
							echo '<div style="background:#fff; text-align:center; padding:20px 0;color:' .  esc_attr( $main_colour ) . '; font-weight:bold">' . esc_html__('THIS INVOICE IS OVERDUE', 'projectopia-core') . '</div>';      
						}
					}
				}
			}
			...
			<?php if ( ! empty($invoice_details['paid']) && $invoice_details['paid'] == true ) { ?>	
				<div style="background:#fff; text-align:center; padding:20px 0;color:<?php echo esc_attr( $main_colour ); ?>; font-weight:bold"><?php esc_html_e('THIS INVOICE HAS BEEN PAID', 'projectopia-core'); ?></div>
			<?php } ?>
			<table style="width:100%;border-collapse:collapse">
				<tr>
					<td style="padding:15px;">
						<span class="invoice_to">
							...
							<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:14px; font-weight:bold"><?php echo esc_html( get_option('secondary_sales_tax_name') ); ?> <?php esc_html_e('Number', 'projectopia-core'); ?>:</span> <span style="font-size:12px"><?php echo esc_html( $stax_reg ); ?></span><br />						
								<?php } ?>
							<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:14px; font-weight:bold"><?php esc_html_e('Invoice #:', 'projectopia-core'); ?></span> <span style="font-size:12px">
							...
							<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:14px; font-weight:bold"><?php esc_html_e('Invoice Date:', 'projectopia-core'); ?></span> <span style="font-size:12px"><?php echo esc_html( $invoice_date ); ?></span><br />
							<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:14px; font-weight:bold"><?php esc_html_e('Due Date:', 'projectopia-core'); ?></span> <span style="font-size:12px">
					...
					<td style="padding:15px">
						<span class="invoice_to">
							<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:14px; font-weight:bold"><?php esc_html_e('INVOICE TO:', 'projectopia-core'); ?></span><br />
							...
							<?php do_action( 'pto_invoice_after_client_address', $post->ID ); ?> <br />
							<?php echo esc_html( $client_postcode ); ?><br />
							<?php if ( ! empty($client_tax_name) ) { ?>
								<?php if ( ! empty($client_tax_name) ) { ?>
									<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:12px; font-weight:bold"><?php 
										/* translators: %s: Tax Name */
										printf(esc_html__('%1$s Reg Number: ', 'projectopia-core'), esc_html( $client_tax_name )); ?></span> <?php echo esc_html( $client_tax_reg ); ?><br />
								<?php } ?>
								<?php if ( ! empty($client_stax_name) ) { ?>
									<span style="color:<?php echo esc_attr( $main_colour ); ?>; font-size:12px; font-weight:bold"><?php 
										/* translators: %s: Secondary Tax Name */
										printf(esc_html__('%1$s Reg Number: ', 'projectopia-core'), esc_html( $client_stax_name )); ?></span> <?php echo esc_html( $client_stax_reg ); ?><br />
								<?php } ?>
							<?php } ?>
						</span>
					</td>
			...
			if ( ! empty($fields) ) {
				echo '<div style="padding:0 15px 15px"><div id="cqpim-custom-fields">';
			...
			<table style="width:100%; border: 0; border-collapse:collapse;text-align:center">
				<thead>
					<tr style="text-align:center;">
						<th style="background:<?php echo esc_attr( $main_colour ); ?>; border:0; color:#fff; padding:10px;  font-size:12px; text-align:center"><?php esc_html_e('Qty', 'projectopia-core'); ?></th>
						<th style="background:<?php echo esc_attr( $main_colour ); ?>; border:0; color:#fff; padding:10px;  font-size:12px"><?php esc_html_e('Description', 'projectopia-core'); ?></th>
						<th style="background:<?php echo esc_attr( $main_colour ); ?>; border:0; color:#fff; padding:10px;  font-size:12px"><?php esc_html_e('Rate', 'projectopia-core'); ?></th>
						<th style="width:100px; background:<?php echo esc_attr( $main_colour ); ?>; border:0; color:#fff; padding:10px;  font-size:12px"><?php esc_html_e('Total', 'projectopia-core'); ?></th>
					</tr>
				</thead>
				<tbody>
				<?php 
				...
				foreach ( $line_items as $item ) { ?>
					<tr style="text-align:center;">
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px; text-align:center"><?php echo esc_html( $item['qty'] ); ?></td>
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo wp_kses_post( $item['desc'] ); ?></td>
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $item['price']) ); ?></td>
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $item['sub']) ); ?></td>
					</tr>
				<?php } ?>
					<tr style="text-align:center;">
						<td style="border:0;" class="no_border" colspan="2" rowspan="6"><div style="font-size:13px;text-align:left"><?php echo wp_kses_post( wpautop($invoice_footer) ); ?></div></td>
						<td style="background:#fff; border:0; color:<?php echo esc_attr( $main_colour ); ?>; padding:10px;  font-size:11px; text-align:right; font-weight:bold"><?php if ( $vat_rate ) { ?><?php esc_html_e('Subtotal:', 'projectopia-core'); ?><?php } else { ?><?php esc_html_e('TOTAL:', 'projectopia-core'); ?><?php } ?></td>
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $sub) ); ?></td>
					</tr>
				<?php 
				$outstanding = $sub;
				if ( $vat_rate ) { 
					...
					<tr style="text-align:center;">
						...
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $vat) ); ?></td>
					</tr>
					<?php if ( ! empty($svat_rate) ) { ?>
						<tr style="text-align:center;">
							...
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $svat) ); ?></td>
						</tr>					
					<?php } ?>
					<tr style="text-align:center;">
						...
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $total) ); ?></td>
					</tr>
				<?php } ?>
					<tr style="text-align:center;">
						...
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px"><?php echo esc_html( pto_calculate_currency($post->ID, $received) ); ?></td>
					</tr>
					<tr style="text-align:center;">
						...
						<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px;  font-size:11px;"><?php echo esc_html( pto_calculate_currency($post->ID, $outstanding) ); ?></td>
					</tr>
				</tbody>
			</table>
			<div style="text-align:center; font-size:12px; padding:10px 0; border-top:1px solid <?php echo esc_attr( $main_colour ); ?>">
				<?php if ( ! empty($company_name) ) { ?>
					<?php echo esc_html( $company_name ); ?> | 
				<?php } ?>
				<?php if ( ! empty($company_telephone) ) { ?>
					<?php echo esc_html( $company_telephone ); ?> | 
				<?php } ?>
				<a style="color:#000" href="mailto:<?php echo esc_attr( $company_accounts_email ); ?>"><?php echo esc_html( $company_accounts_email ); ?></a>
			</div>
		<?php } else { ?>
