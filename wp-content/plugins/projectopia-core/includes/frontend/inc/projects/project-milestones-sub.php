	...
	<div class="cqpim-dash-item-full grid-item tasks-box">
		<div class="cqpim_block">
			<div class="cqpim_block_title">
				<div class="caption">
					<span class="caption-subject font-light-violet sbold"><?php esc_html_e('Milestones & Tasks', 'projectopia-core'); ?></span>
				</div>	
			</div>
			...
			$cost = preg_replace("/[^\\d.]+/","", $element['cost']); 
				...
				foreach ( $ordered as $task ) {
					...
					foreach ( $subordered as $subtask ) {
						...
						$client = false;
						if ( $task_owner == $client_id ) {
							$client = true;
						}
						if ( $task_owner ) {
							if ( $client == true ) {
								$nid = preg_replace("/[^0-9,.]/", "", $task_owner);
								$client_object = get_user_by('id', $nid);
								$task_owner = ' (' . esc_html( $client_contact_name ) . ')';
							} else {
								$team_details = get_post_meta($task_owner, 'team_details', true);
								$team_name = isset($team_details['team_name']) ? $team_details['team_name'] : '';
