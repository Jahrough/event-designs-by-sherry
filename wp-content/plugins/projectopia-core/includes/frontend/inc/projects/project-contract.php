		...
		<div id="cqpim_admin_title">
			<?php if ( current_user_can( 'cqpim_view_project_contract' ) OR $client_user_id == $user_id OR in_array($user->ID, $project_client_ids) ) { ?>	
				<?php 
					$p_title = get_the_title(); 
					$p_title = str_replace('Private:', '', $p_title); 
					echo '<a href="' . esc_url(get_the_permalink($client_dash)) . '">' . esc_html__('Dashboard', 'projectopia-core') . '</a> <i class="fa fa-circle"></i> <a href="' . esc_url(add_query_arg('pto-page', 'projects', get_the_permalink($client_dash))) . '">' . esc_html__('Projects', 'projectopia-core') . '</a> <i class="fa fa-circle"></i> ' . esc_html($p_title); 
				?>							
			<?php } else {
		...
		<div class="cqpim-dash-item-full grid-item">
			<div class="cqpim_block quote_layout">
				<div class="cqpim_block_title">
					...
					<div class="actions">
						<a class="cqpim_button cqpim_small_button border-green-sharp font-light-violet rounded_2 sbold" href="<?php echo esc_url(add_query_arg('pto-page', 'contract-print', get_the_permalink())); ?>" target="_blank"><?php esc_html_e('VIEW PRINTABLE VERSION', 'projectopia-core'); ?></a>
					</div>
				</div>					
				<div class="quote_logo">
					<img src="<?php echo esc_url( $logo_url ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" />
				</div>
