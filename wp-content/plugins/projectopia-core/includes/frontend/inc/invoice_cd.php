...
<div class="cqpim-dash-content">
	...
	<div id="cqpim-dash-content">
		<div id="cqpim_admin_title">
			...
			$p_title = get_the_title();
			$p_title = str_replace('Protected:', '', $p_title);
			echo '<a href="' . esc_url(get_the_permalink($client_dash)) . '">' . esc_html__('Dashboard', 'projectopia-core') . '</a> <i class="fa fa-circle"></i> <a href="' . esc_url(add_query_arg('pto-page', 'invoices', get_the_permalink($client_dash))) . '">' . esc_html__('Invoices', 'projectopia-core') . '</a> <i class="fa fa-circle"></i> ' . esc_html($p_title);
			} else {
			...
			<div class="masonry-grid">
				...
				<div class="cqpim-dash-item-double grid-item">
					<div class="cqpim_block">
											...
											<ul>
												...
												<?php if ( ! empty( $stripe ) ) { ?>
													<li>
														...
														$client_company = !empty( $client_details['client_company'] ) ? $client_details['client_company'] : '';
														$client_contact = !empty( $client_details['client_contact'] ) ? $client_details['client_contact'] : '';
														$client_address = !empty( $client_details['client_address'] ) ? $client_details['client_address'] : '—';
														$client_postcode = !empty( $client_details['client_postcode'] ) ? $client_details['client_postcode'] : '';
														$client_telephone = !empty( $client_details['client_telephone'] ) ? $client_details['client_telephone'] : '';
														$client_email = !empty( $client_details['client_email'] ) ? $client_details['client_email'] : $c_user->user_email;
						...
						<?php } ?>
						<?php
							$print_url = add_query_arg('pto-page', 'print', get_the_permalink());
							$pdf_url = add_query_arg('download_pdf', '1', get_the_permalink());
                        ?>
                        <a class="cqpim_button cqpim_button_link font-white bg-violet block mt-10 rounded_2 block" href="<?php echo esc_url($print_url); ?>" target="_blank"><?php esc_html_e('View Printable Invoice', 'projectopia-core'); ?></a>
                        <a class="cqpim_button cqpim_button_link font-white bg-violet block mt-10 rounded_2 block" href="<?php echo esc_url($pdf_url); ?>"><?php esc_html_e('Download PDF Invoice', 'projectopia-core'); ?></a>
					</div>
				</div>		
			</div>
			<?php if ( ! empty($ideal) ) { 
