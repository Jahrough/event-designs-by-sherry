...
<div>
	<div id="cqpim-new-message" class="cqpim_block" style="display:none">
		<div class="cqpim_block_title">
			<div class="caption">
				<i class="fa fa-envelope-open font-light-violet" aria-hidden="true"></i>
				<span class="caption-subject font-light-violet sbold"><?php esc_html_e('New Conversation', 'projectopia-core'); ?></span>
			</div>
			<div class="actions">
	...
	<div id="cqpim-reply-message" class="cqpim_block" style="display:none">
		<div class="cqpim_block_title">
			<div class="caption">
				<i class="fa fa-envelope-open font-light-violet" aria-hidden="true"></i>
				<span class="caption-subject font-light-violet sbold"><?php esc_html_e('Reply to Conversation', 'projectopia-core'); ?></span>
			</div>
			<div class="actions">
	...
	<div class="cqpim_block">
		<div class="cqpim_block_title">
			<div class="caption">
				...
				<?php if ( ! empty($conversation) ) { ?>
					<span class="caption-subject font-light-violet sbold"><?php echo esc_html( str_replace('Private: ', '', get_the_title($conversation->ID)) ); ?></span>
				<?php } else { ?>
					<span class="caption-subject font-light-violet sbold"><?php esc_html_e('My Messages', 'projectopia-core'); ?></span>
				<?php } ?>
			</div>
			<div class="actions">
		...
		<?php } else { ?>
			...
			<?php } else { ?>
				<table class="dataTable datatable_style milestones" id="front_msgs_table"> 
					<thead>
						<tr>
					...
					<tbody>
						<?php foreach ( $conversations as $conversation ) { 
							...
							<tr <?php if ( empty($read_val) ) { echo ' class="cqpim-unread"'; } ?>>
								<td><?php if ( empty($read_val) ) { echo '<i class="fa fa-envelope" aria-hidden="true"></i> '; } else { echo '<i class="fa fa-envelope-open" aria-hidden="true"></i> '; } ?>&nbsp;&nbsp;<a href="<?php echo esc_url(add_query_arg(array('pto-page' => 'messages', 'conversation' => $n_id), get_the_permalink($client_dash))); ?>"><?php echo esc_html( str_replace('Private: ','', get_the_title($conversation->ID)) ); ?></a></td>
								...
								<td>
									<?php foreach ( $members as $member ) {
										...
										$recip_is     = ! empty( $recip->ID ) ? $recip->ID : '';
										$display_name = ! empty( $recip->display_name ) ? $recip->display_name : '';
										echo '<div class="cqpim-messagelist-avatar">';
										echo get_avatar( $recip_is, 80, '', $display_name, array( 'force_display' => true ));
										echo '</div>';
									} ?>
								</td>
	...
	$text = __('Search for team member name...', 'projectopia-core'); ?>
		<script>
			jQuery(document).ready(function() {
				jQuery('#to').tokenInput(<?php echo wp_json_encode( $p_search ); ?>, {
		...
		<script>
			jQuery(document).ready(function() {
				jQuery('#ato').tokenInput(<?php echo wp_json_encode( $p_search ); ?>, {
					hintText: "<?php echo esc_js( $text ); ?>",
					noResultsText: '<?php esc_html_e('No Results', 'projectopia-core'); ?>',
					searchingText: '<?php esc_html_e('Searching', 'projectopia-core'); ?>'
				});						
