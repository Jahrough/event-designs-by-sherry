...
<div id="content" role="main">	
	...
	if ( $logo ) { ?>
		<div style="text-align:center; max-width:400px; margin:0 auto">
			<img style="max-width:100%; margin:20px 0 0" src="<?php echo esc_url( $logo['cqpim_dash_logo'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" />
		</div>
	<?php } ?>
	<br /><br />
			...
			<?php } else { ?>
				<form id="reset_pass_conf">
					...
					<input type="hidden" id="hash" value="<?php 
						  // phpcs:ignore WordPress.Security.NonceVerification.Recommended
						  echo esc_attr(sanitize_text_field(wp_unslash($_GET['h']))); ?>" />
