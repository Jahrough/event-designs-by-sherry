	...
	<div id="cqpim-dash-content">
		<div id="cqpim_admin_title">
			<?php if ( $assigned == $client_id ) {
				...
				echo '<a href="' . esc_url( get_the_permalink($client_dash) ) . '">' . esc_html__('Dashboard', 'projectopia-core') . '</a> <i class="fa fa-circle"></i> <a href="' . esc_url(add_query_arg(
					array(
						'pto-page' => 'subscriptions'
					),
					get_the_permalink($client_dash)
				)) . '">' . esc_html__('Subscriptions', 'projectopia-core') . '</a> <i class="fa fa-circle"></i> ' . esc_html( $p_title );
				} else {
