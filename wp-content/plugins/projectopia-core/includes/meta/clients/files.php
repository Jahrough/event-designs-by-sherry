...
function pto_client_files_metabox_callback( $post ) {
	...
	if ( ! $all_attached_files ) {
		...
		foreach ( $all_attached_files as $file ) {
			...
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
			$delete_link = sprintf('<button href="%s" class="delete_file btn" data-id="%s"><img src="%s" alt="delete" class="img-fluid"/></button>', esc_url($file->guid), $file->ID, PTO_PLUGIN_URL . '/assets/admin/img/trash.svg');
