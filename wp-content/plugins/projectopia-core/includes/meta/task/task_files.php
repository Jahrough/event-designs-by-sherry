...
function pto_task_files_metabox_callback( $post ) {
	...
	if ( ! $all_attached_files ) {
		...
		foreach ( $all_attached_files as $file ) {
			...
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
			$download_link = sprintf('<a href="%s" download="%s" class="btn"><img src="%s" alt="download" class="img-fluid"/></a>', $file->guid, $file->post_title, PTO_PLUGIN_URL . '/assets/admin/img/download.svg');

			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
			$delete_link = sprintf('<button href="%s" class="delete_file btn" data-id="%s"><img src="%s" alt="delete" class="img-fluid"/></button>', $file->guid, $file->ID, PTO_PLUGIN_URL . '/assets/admin/img/trash.svg');
