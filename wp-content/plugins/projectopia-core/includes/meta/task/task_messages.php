...
function pto_task_messages_metabox_callback( $post ) {
	...
	if ( ! empty( $messages ) ) {
		...
		foreach ( (array) $project_messages as $project_id => $project_message ) {
			...
			$delete_link = sprintf('<button class="delete_message btn" data-id="%s" style="margin-top: -5px;"><img src="%s" alt="delete" class="img-fluid"/></button>', $project_id, esc_url(PTO_PLUGIN_URL . '/assets/admin/img/trash.svg'));
