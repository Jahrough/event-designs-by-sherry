...
function pto_invoice_client_project_metabox_callback( $post ) {
	...
	if ( $post->post_name ) {
		$base_url = get_the_permalink($post->ID);
        $edit_url = get_edit_post_link($post->ID);
        
        $print_url = add_query_arg('pto-page', 'print', $base_url);
        $pdf_url = add_query_arg('download_pdf', '1', $edit_url);
        
        echo '<a class="piaBtn btn btn-primary btn-block mt-2" href="' . esc_url($print_url) . '" target="_blank">' . esc_html__('View Printable Invoice', 'projectopia-core') . '</a>';
