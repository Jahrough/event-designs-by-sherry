...
function pto_quote_messages_metabox_callback( $post ) {
	...
	if ( ! empty( $quote_messages ) ) {
		...
		foreach ( (array) $quote_messages as $quote_id => $quote_message ) {
			...
			$delete_link = sprintf('<button class="delete_message btn" data-id="%s" style="margin-top: -5px;"><img src="%s" alt="delete" class="img-fluid"/></button>', $quote_id, PTO_PLUGIN_URL . '/assets/admin/img/trash.svg');
