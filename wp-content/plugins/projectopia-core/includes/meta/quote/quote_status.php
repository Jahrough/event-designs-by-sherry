	...
	if ( ! $quote_accepted ) {
		if ( empty($quote_sent) ) {
			...
			if ( current_user_can('cqpim_send_quotes') ) { 
				...
				echo '<a class="cqpim_button_link piaBtn btn btn-primary btn-block mt-2" href="' . esc_url(add_query_arg('pto-page', 'print', $url)) . '" target="_blank">' . esc_html__('Preview Quote / Estimate', 'projectopia-core') . '</a>';
			}
		}
		if ( $quote_sent ) {
