...
function pto_team_projects_metabox_callback( $post ) {
	...
	if ( $projects ) {
		$i = 0;
		?><div class="card p-0 m-0"><table class="p-0 piaTableData table-responsive-lg table table-bordered w-100 dataTable" id="front_teamprojects_table" data-ordering="false" data-rows="10">
		<?php
		echo '<thead>';
		...
		foreach ( $projects as $project ) {
			...
			if ( in_array($post->ID, $project_contributors_id) ) {
				...
				echo '<tr>';
				echo '<td><a href="' . esc_url( $project_edit ) . '" target="_blank">' . esc_html( $project_title ) . '</a></td>';
		...
		echo '</table></div>';
		if ( $i == 0 ) {
			echo '<div class="cqpim-alert cqpim-alert-info alert-display">' . esc_html__('This team member has not been assigned to any projects.', 'projectopia-core') . '</div>';
		}
	} else {
