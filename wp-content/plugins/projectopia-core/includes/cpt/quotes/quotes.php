	...
	switch ( $column ) {
		...
		case 'q_status':
			...
			if ( $converted ) {
				...
				printf(
					wp_kses(
						/* translators: %s: Link to the converted project */
						__('This was converted to the Project %s', 'projectopia-core'),
						array('a' => array('href' => array(), 'target' => array()))
					),
					'<a href="' . esc_url(get_edit_post_link($converted)) . '" target="_blank">#' . esc_html($converted) . '</a>'
				);
				echo '</div>';
			}
		    break;
